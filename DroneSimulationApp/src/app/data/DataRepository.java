...
public class DataRepository implements IDataProvider {
    ...
    private DataRepository() {
        ...
        // Map each drone type ID to its object for quick access
        for (DroneType dt : droneTypes) {
            droneTypeMap.put(dt.getId(), dt);
        }
    ...
    /**
     * Returns a DroneType by its ID.
     * @param id the drone type ID
     * @return DroneType object or null if not found
     */
    public DroneType getDroneTypeById(int id) {
        return droneTypeMap.get(id);
    }

    /**
     * Returns a DroneType by its reference URL string.
     * @param droneTypeRef string containing drone type reference (URL or numeric)
     * @return DroneType object or null if not found or parse error
     */
    public DroneType getDroneTypeByUrl(String droneTypeRef) {
        try {
            int id = Integer.parseInt(droneTypeRef.replaceAll("[^0-9]", ""));
